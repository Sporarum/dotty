...
For this expression to be well-typed, the method must be *applicable* to its arguments, which is defined next by case analysis on ´f´'s type.
...
If the method part ´e´ is of some value type, the type application is taken to be equivalent to `´e´.apply[´T_1 , ...,´ T´_n´]`, i.e. the application of an `apply` method defined by ´e´.
